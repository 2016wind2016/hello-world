...
class RTSpeedLeaderboard extends RTLeaderboard {
  ...
  async rank(id, keyRank = this.keyRank(), keyData = this.keyData()) {
    ...
    let rank = await this._conn.zrank(keyRank, id);
    if (rank === null) {
      return -1;
    }
    rank += 1;
    if (rank > this.maxNum) {
      ...
      this._conn.zremrangebyrank(keyRank, this.maxNum, -1);
    }
    return rank;
  }
  ...
  async range(start, stop, keyRank = this.keyRank(), keyData = this.keyData()) {
    ...
    let list = [];
    let ids = await this._conn.zrange(keyRank, start, stop);
...
module.exports = RTSpeedLeaderboard;